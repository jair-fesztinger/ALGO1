...
Beta = OLS_Method.params[0] #intercept in the linear regression equation
...
#logic test
if ADF_Test[0] <= ADF_Test[4]['5%'] and ADF_Test[1] <= .05:
...
#Bollinger Bands 
#calculating the moving average and the standard deviation of the asset's prices
...
fig_rsi.add_scatter(x=returns['time'], y=[67]*len(returns), mode='lines', name='Overbought (67)', line=dict(dash='dash', color='red'))
fig_rsi.add_scatter(x=returns['time'], y=[33]*len(returns), mode='lines', name='Oversold (33)', line=dict(dash='dash', color='green'))

fig_rsi.show()

#Generate Trade Signal 
def trade_strategy(data, spread_cl='spread', upper_cl='Upper', lower_cl='Lower', spread_RSI_cl='Spread_RSI', threshold=33):
...
mean_threshold = .2
crossed_mean = False #track if the spread has crossed the mean after opening
...
for i in range(len(returns)):
    ...
    # Check if the spread crosses the mean for the first time
    elif position == 1 and not crossed_mean and spread > mean_band:
        crossed_mean = True


    #Close a long position: Signal = -1 or neutral
    elif position == 1 and crossed_mean and (spread >= upper_band or (spread > mean_band and spread <= mean_band + mean_threshold)):
        ...
        position_log.append({
            ...
            'Long Asset': 'AAPL',
            'Short Asset': 'MSFT',
            'Exit Price Long': entry_price_AAPL,
            'Exit Price Short': entry_price_MSFT,
            'PnL': pnl_trade 
        })
    ...
    elif position == -1 and crossed_mean and (spread <= lower_band or (spread < mean_band and spread >= mean_band - mean_threshold)):
        exit_price_AAPL = data['AAPL'].iloc[i]
        exit_price_MSFT = data['MSFT'].iloc[i]
...
position_16 = position_df.head(16)
print(position_16)
print('---')
